...
#' @export
FGLS_wrapper_binary <- function(proband_ids,K,pheno,method="PAFGRS",thr=NULL,w=NULL,h2=NULL,env_cor_s=1,env_cor_f=1,env_cor_m=1,sib_mat=NULL,father_mat=NULL,mother_mat=NULL){
  ...
  ghat <- sapply(rels_and_self[,V1], function(k){
    ...
    if(method=="OK") { 
      ...
      } else  if(method=="PAFGRS") { out1 <- pa_fgrs(rel_status = pheno_k$aff[-1],rel_thr =pheno_k$thr[-1],rel_w = pheno_k$w[-1],covmat = as.matrix(covmat)) } else  if(method=="accuracy") {if(length(k)>1 & any(rowSums(pheno_k[-c(1),-c("id","thr")])>0)) {
